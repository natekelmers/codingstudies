...
def how_many_airlines(a):
  return len(a.split(","))
 
''' How many airlines start with the letter 'A' (only capital letters) '''
def how_many_airlines_letter_a(a):
  airlines_clean = [a1.strip() for a1 in a.split(",")]
  count = 0
  for air in airlines_clean:
    if air[0:1] == "A":
      count += 1
  return count
        
''' create a list that only includes airlines that start with the letter "A" '''
def list_of_airlines_letter_a(a):
  airlines_clean = [a1.strip() for a1 in a.split(",")]
  airlines_clean_a_only = [a2 for a2 in airlines_clean if a2[0:1] == "A"]
  return airlines_clean_a_only

''' Create a list (of the same length) that contains 1 if there's a star and 0 if not.
Expected output: [0, 1, 0, ...] '''
def list_of_airlines_star(a):
  airlines_clean = [a1.strip() for a1 in a.split(",")]
  airlines_clean_star = []
  for i in airlines_clean:
    if i.find("*") > -1:
      airlines_clean_star.append(1)
    else:
      airlines_clean_star.append(0)
  return airlines_clean_star
...
def list_of_airlines_no_star(a):
  return [a1.replace("\n", "").replace("*", "") for a1 in a.split(",")]
...
''' def dictionary_of_airlines(a):
  import csv
  with open("airline_safety.csv") as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
      return(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7])
      
print dictionary_of_airlines(airlines)'''

### My solution would have involved adding the values in columns C and F (for
### incidents_85_99 and incidents_00_14) and dividing their total by 
### 33 (2014-1985 = 33 years) and then adding them to a dictionary with the
### value in column A (the airline name). I can't seem to work out how to do
### this yet on my own though!